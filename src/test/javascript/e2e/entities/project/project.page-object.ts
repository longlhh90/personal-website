...
export class ProjectUpdatePage {
  ...
  demoURLInput = element(by.id('field_demoURL'));
  ...
  async setDemoURLInput(demoURL: string): Promise<void> {
    await this.demoURLInput.sendKeys(demoURL);
  }

  async getDemoURLInput(): Promise<string> {
    return await this.demoURLInput.getAttribute('value');
  }
